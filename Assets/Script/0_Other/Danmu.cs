...
namespace TouhouMachineLearningSummary.Other
{
    public class Danmu : OdinEditorWindow
    {
        ...
        {
            string cookiedata = File.ReadAllText(@"Assets\Script\0_Other\cookie.txt");
            ...
            request.AddOrUpdateParameter("csrf_token", File.ReadAllLines(@"Assets\Script\0_Other\info.txt")[1]);
            request.AddOrUpdateParameter("csrf", File.ReadAllLines(@"Assets\Script\0_Other\info.txt")[1]);
        }
